...
在 **Releases** 页面下载：**列表最上面一条就是最新镜像**。每次构建都会新建一条发布，
tag 形如 `v25.12.5-x86_64.20260920`（末尾是构建日期，同日多次构建会带 `-2`、`-3` 序号），
所以有没有新版本一眼就能看出来。每条发布提供**一个**整盘镜像，构建时间、提交、`sha256`
与文件大小都写在该发布说明里，可用来核对手里的文件对不对。
...
并把镜像发到 Release：tag 为 `v<版本>-<配置>.<构建日期>`，**每次构建都是一条新发布**，
因此不会出现「资产被悄悄替换、看不出来更新过」的情况。每个目标只保留最近 5 条发布，
更旧的连同 tag 一起自动清理。
